...
public final class LitematicaHelper {
    ...
    /**
     * Active placement: selected first, otherwise first enabled placement.
     * Never uses player position.
     */
    public static Optional<SchematicPlacement> getActivePlacement() {
        ...
        try {
            ...
            SchematicPlacement selected = mgr.getSelectedSchematicPlacement();
    ...
    /**
     * Schematic + absolute world origin for the active placement.
     * Origin comes from the placement (rotation/mirror already applied in getSchematic).
     */
    public static Optional<Tuple<IStaticSchematic, Vec3i>> getActiveSchematic() {
        Optional<SchematicPlacement> opt = getActivePlacement();
        if (!opt.isPresent()) {
            return Optional.empty();
        }
        List<SchematicPlacement> all = DataManager.getSchematicPlacementManager().getAllSchematicsPlacements();
        int idx = all.indexOf(opt.get());
        if (idx < 0) {
            return Optional.empty();
        }
        return Optional.of(getSchematic(idx));
    }

    /**
     * Real Litematica render layer range. Failures are treated as "unlocked" (allow all).
     */
    public static boolean isInRenderLayer(int worldX, int worldY, int worldZ) {
        try {
            LayerRange range = DataManager.getRenderLayerRange();
            return range.isPositionWithinRange(worldX, worldY, worldZ);
        } catch (Throwable t) {
            return true;
        }
    }

    public static int getCurrentLayerY() {
        try {
            return DataManager.getRenderLayerRange().getCurrentLayerInteger();
        } catch (Throwable t) {
            return 0;
        }
    }
     ...
     * @return  The transformed schematic and the position of its minimum corner (absolute world coords)
     */
