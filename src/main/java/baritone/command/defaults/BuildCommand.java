...
public class BuildCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        // No arguments → AutoBuild the active Litematica placement
        if (!args.hasAny()) {
            boolean ok = baritone.getBuilderProcess().buildActiveLitematic();
            if (!ok) {
                throw new CommandInvalidStateException(
                        "No enabled Litematica placement found. Place and select/enable one in Litematica first.");
            }
            logDirect("AutoBuild started from active Litematica placement");
            return;
        }
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Build a schematic from the active Litematica placement or from a file.",
                "",
                "Usage:",
                "> build — Build currently selected/enabled Litematica placement",
